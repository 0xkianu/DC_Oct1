...
function findMultipleBreeds() {
    let arrayOfMultipleDogs = [];
    for(let i=0; i < arrayOfDogs.length; i++) {
        if(arrayOfDogs[i].breed.length > 1) {
            arrayOfMultipleDogs.push(arrayOfDogs[i]);
        }
    }
    return(arrayOfMultipleDogs);   
}

//const arrLargeDogs = largeDogs();
//console.log(arrLargeDogs);

const arrMulitBreeds = findMultipleBreeds();
console.log(arrMulitBreeds);